...
let () = assert (Tyre.exec dim_re "dim:23x10" = Result.Ok (23, 10))
...
let nice_dim : (_, dim) Tyre.t =
  Tyre.conv
    (fun (x, y) -> {x; y})
    (fun {x; y} -> (x, y) )
      (* We provide the backward transformation to unparse *)
    dim
...
let () =
  assert (
    Tyre.eval list_of_dims [{x= 2; y= 3}; {x= 12; y= 54}] = "dims:2x3;12x54;" )
