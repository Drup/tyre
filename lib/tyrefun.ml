...
module T = struct
  ...
  type 'a raw =
    ...
    | Regexp : Regex.t * Regex.re Lazy.t -> Regex.string raw
    ...
    | Mod    : (Regex.t -> Regex.t) * 'a raw -> 'a raw

  type _ wit =
    | Lit    : int -> Regex.string wit
    | Conv   : 'a wit * ('a, 'b) conv -> 'b wit
    | Opt    : Regex.Idx.idx * 'a wit -> 'a option wit
    | Alt    : Regex.Idx.idx * 'a wit * Regex.Idx.idx * 'b wit
        -> [`Left of 'a | `Right of 'b] wit
    | Seq    :
        'a wit * 'b wit -> ('a * 'b) wit
    | Rep   : int * 'a wit * Regex.re -> 'a Seq.t wit
...
let regex x : _ t =
  let re = lazy Regex.(compile @@ whole_string @@ no_group x) in
  Regexp (x, re)
...
(* [modifier] is unsafe in general (for example [modifier Regex.group]).
...
let rec witnesspp
  ...
  = fun ppf tre -> let open T in match tre with
    | Regexp (re, _) -> Regex.pp_string ppf @@ Regex.witness re
...
(** {3 Regexp construction}
    ...
    Each alternative is marked with {!Regex.mark}. We store the markid in order
    to be able to guess the branch matched.
*)

let rec build
  : type a. int -> a t -> int * a T.wit * Regex.t
  = let open! Regex in let open T in
  fun i -> function
    | Regexp (re, _) ->
    ...
    | Rep e ->
      ...
      (i+1), Rep (i,w,Regex.compile re), group @@ rep @@ no_group re
    | Mod (f, e) ->
      let i', w, re = build i e in
      i', w, f re
...
let[@specialize] rec extract
  : type a. original:Regex.string -> a T.wit -> Regex.Group.t -> a
  = fun ~original rea s -> let open T in match rea with
    | Lit i -> Regex.Group.get s i
    | Conv (w, conv) ->
      let v = extract ~original w s in
      conv.to_ v
    | Opt (id,w) ->
      if not @@ Regex.Idx.test s id then None
      else Some (extract ~original w s)
    | Alt (i1,w1,i2,w2) ->
      if Regex.Idx.test s i1 then
        `Left (extract ~original w1 s)
      else if Regex.Idx.test s i2 then
...
and[@specialize] extract_list
  : type a. original:Regex.string -> a T.wit -> Regex.re -> int -> Regex.Group.t -> a Seq.t
  = fun ~original e re i s ->
    let aux = extract ~original e in
    let (pos, pos') = Regex.Group.offset s i in
    let len = pos' - pos in
    Seq.map aux @@ Regex.all ~pos ~len re original
...
type 'r wit_route =
    WRoute : Regex.Idx.idx * 'a T.wit * ('a -> 'r) -> 'r wit_route
...
let rec build_route_aux i rel wl = function
  ...
  | Route (tre, f) :: l ->
    let i', wit, id, re = Regex.Idx.with_mark build i tre in
...
let rec extract_route ~original wl subs = match wl with
  | [] ->
    ...
    assert false
  | WRoute (id, wit, f) :: wl ->
    if Regex.Idx.test subs id then
...
type 'a re = { info : 'a info ; cre : Regex.re }

let compile tre =
  let _, wit, re = build 1 tre in
  let cre = Regex.compile re in
  { info = One wit ; cre }

let route l =
  let rel, wl = build_route l in
  let cre = Regex.compile @@ Regex.alt rel in
  { info = Routes wl ; cre }
...
let[@inline] exec ?pos ?len ({ info ; cre } as tcre) original =
  match Regex.exec ?pos ?len cre original with
...
let execp ?pos ?len {cre ; _ } original =
  Regex.test ?pos ?len cre original

let all_seq ?pos ?len { info ; cre } original =
  let seq = Regex.all ?pos ?len cre original in
...
let rec pp
  ...
  = fun ppf -> let open T in function
      | Regexp (re,_) -> sexp ppf "Re" "%a" Regex.pp re
...
let rec pp_wit
  ...
  = fun ppf -> let open T in function
      ...
      | Rep (i, w, re) -> sexp ppf "Rep" "%i@ %a@ %a" i pp_wit w Regex.pp_re re
...
let pp_re ppf = function
  | { info = One w; cre } ->
    sexp ppf "One" "%a@ %a" Regex.pp_re cre pp_wit w
  | { info = Routes wl; cre } ->
    sexp ppf "Route" "%a@ %a" Regex.pp_re cre (pp_list pp_wit_route) wl
